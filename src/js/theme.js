...
<div class="fixed right-5 bottom-5" id="theme-switcher">
  <button
      ...
      <a class="text-sm py-2 px-3 font-normal block w-full whitespace-nowrap bg-transparent text-gray-700 dark:text-gray-100 hover:bg-gray-100 disabled:text-gray-400 disabled:pointer-events-none disabled:bg-transparent active:no-underline active:text-zinc-800 dark:hover:bg-gray-600 focus:outline-none focus:bg-gray-200 focus:dark:bg-gray-600"
        ...
        <div class="pointer-events-none">
          <div class="inline-block w-[24px] text-center" data-theme-icon="system">
            <svg xmlns="http://www.w3.org/2000/svg" aria-hidden="true" focusable="false"
              class="inline-block w-4" role="img" viewBox="0 0 640 512">
              <path fill="currentColor"
...
document.body.insertAdjacentHTML('beforeend', switcherElement);

const themeSwitcher = document.querySelector('#theme-switcher');
const themeSwitcherButton = themeSwitcher.querySelector('button');
const themeSwitcherItems = themeSwitcher.querySelectorAll('a');

const setSystemTheme = () => {
  if (window.matchMedia('(prefers-color-scheme: dark)').matches) {
    document.documentElement.classList.add('dark');
    setActiveThemeIcon('dark');
  } else {
    document.documentElement.classList.remove('dark');
    setActiveThemeIcon('light');
  }
  setActiveDropdownItem('system');
};
...
const setLightTheme = () => {
  ...
  setActiveThemeIcon('light');
  setActiveDropdownItem('light');
};

const setActiveThemeIcon = (theme) => {
  themeSwitcherButton.innerHTML = themeSwitcher.querySelector(
    `[data-theme-icon=${theme}]`
  ).innerHTML;
};

const setActiveDropdownItem = (theme) => {
  themeSwitcher.querySelectorAll('[data-theme-icon]').forEach((item) => {
    item.classList.remove('text-blue-500');
  });
  themeSwitcher.querySelectorAll('[data-theme-name]').forEach((item) => {
    item.classList.remove('text-blue-500');
  });
  themeSwitcher.querySelector(`[data-theme-icon=${theme}]`).classList.add('text-blue-500');
  themeSwitcher.querySelector(`[data-theme-name=${theme}]`).classList.add('text-blue-500');
};
